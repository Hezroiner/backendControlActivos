...
export class ActivoService {
  ...
  async getAllActivos(disponibilidad?: string): Promise<Activo[]> {
    try {
      const relations = [ 'ubicacion', 'licitacion', 'licitacion.ley', 'licitacion.proveedor' ];
      ...
      if (disponibilidad) {
        ...
        return await this.activoRepository.find({ relations });
      }
    } catch (error) {
      throw new NotFoundException('No se encontraron los activos');
    }
  }
